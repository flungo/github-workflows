...
# existing — nor on the branches the links name, which are never fetched. The
# major numbers below are invented for the test.
set -euo pipefail
cd "$(dirname "$0")"

script=./build.sh
guide_path=docs/reference/upgrading.md

# The link a section is expected to carry: its own major's branch, not main.
section_url() {
  echo "https://github.com/flungo/github-workflows/blob/v$1/$guide_path#v$1"
}
...
has "v2 section link" "- [\`v2\`]($(section_url 2))"
has "v3 section link" "- [\`v3\`]($(section_url 3))"
hasnt "no section for the pinned major itself" "($(section_url 1))"
...
# Each hop is read on its own major's branch: main moves on, and the docs a
# section links onward to resolve against whichever ref it was opened at.
hasnt "never linked on main" "/blob/main/"
...
hasnt "no sections when up to date" "/blob/v"
...
has "v2 section link" "- [\`v2\`]($(section_url 2))"
hasnt "no section for the settling major" "($(section_url 3))"
