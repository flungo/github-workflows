...
| [013](013-per-major-upgrade-guide.md) | A per-major upgrade guide, scoped to breaking changes only | Proposed | ADR-004's version-check issue tells a consumer it is on a frozen major and nothing about how to catch up, leaving it to reconstruct the migration from commit history organised by when work happened rather than by what it must do. `docs/reference/upgrading.md` gets one section per major, and adding it becomes a step in cutting one — in the same pull request as the breaking change, while whoever made it still holds the context. Deliberately **not** a changelog: the name is the scope control, and the boundary is what a consumer can act on. A non-breaking change arrives automatically and cannot be declined, so an entry for it would address someone with nothing to do; a breaking change does not arrive at all until they act. The version-check issue links to the guide. |
| [014](014-grace-period-before-prompting-a-new-major.md) | A grace period before consumers are prompted onto a new major | Accepted | Cutting a major declares its contract final and tells every consumer to migrate in the same instant, but a contract is only proven by adopting it — so a mistake found on day two costs either a mega-PR that got the whole major right first time, or a `v3` for a wrong check name. Those are two events, and only the freeze has to happen at the cut: a new major now gets **7 days** in which nobody is prompted onto it and it may still take a breaking correction in place. `version-check.yml` prompts against the newest major whose grace period has passed, dating the cut from the oldest commit the new major has that the previous one does not (nothing records when a branch appeared); undatable counts as spent, so it fails towards the prompt. Fixed, not an input — the grace period is a property of the release. Ships on `v1`, because a frozen consumer runs the frozen major's copy of the check. |
