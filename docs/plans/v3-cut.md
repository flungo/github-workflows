...
### 4. `terraform.yml`: a formatting failure blocks the merge, as its own `format` job

**Class: breaking** (default-behaviour change + a new check context), found in this review.
**Decided 2026-09-17: in — bad formatting should block a merge.**
Revised 2026-09-20: enforced as a job of its own rather than a failure sequenced inside the plan.
...
In v3, the check is its own `format` job in the [item 6](#6-terraformyml-split-the-terraform-job-into-format-plan-and-apply) split, reporting **`terraform / format`** and joining the required set — bad formatting becomes a red required check, not a footnote in a comment.
The job ID is `format`, not the subcommand's `fmt`: per [ADR-011](../decisions/011-reusable-job-ids-are-the-check-name.md) a job ID names the *role*, and the family precedent already reads that way — the provider job that runs `gofmt` is `lint`, not `gofmt` — so the required-check string stays vendor-neutral and legible rather than echoing Terraform's abbreviation.
It is the family's `terraform-provider-test.yml` `lint` precedent, and the cheapest, least-privileged job in the file: a checkout, `setup-terraform`, and `terraform fmt -check` — no init, no backend, and **no secrets at all** — running on `pull_request` alongside `plan` (a skipped check satisfies a required context, so push runs need not carry it).
The plan comment's outcome table drops its formatting column: the check itself is the visible, blocking signal.
No current consumer is knowingly format-dirty, so the expected migration cost is zero beyond the required-check string; the change is behavioural, hence a major.
No opt-out input is added — a consumer that wants non-blocking formatting is a consumer whose formatting drifts, and an input can be added additively later if a real case appears.
...
### 6. `terraform.yml`: split the `terraform` job into `format`, `plan` and `apply`

**Class: breaking** (check-context change).
Added 2026-09-19, when an audit of the post-v2 workflows found the one naming inconsistency the v2 cut left behind; revised 2026-09-20 from a plain job rename to a job split.
...
**The shape: event-partitioned jobs, the pattern `markdown-links.yml`'s `internal`/`external` already set.**
A `plan` job runs on `pull_request` (and `workflow_dispatch` with `operation: plan`) and owns the PR comment; an `apply` job runs on push to the default branch (and dispatch with `operation: apply`, guarded per scope item 5) and plans fresh then applies within the one job — exactly what the single job does on a push today, so plan-apply fidelity within a run is unchanged and nothing crosses a job boundary.
A `format` job (scope item 4) runs on `pull_request` in parallel with `plan`, carrying the formatting gate as its own secret-free check.
Each event runs one state-touching job, so there is no second checkout of the backend path, no double init, and no extra lock cycle on any run.
The caller half stays `terraform` (the filename, [ADR-010](../decisions/010-caller-job-ids-match-the-workflow-filename.md)), so the contexts become **`terraform / format`** and **`terraform / plan`** on pull requests and **`terraform / apply`** on pushes.
Beyond the role names, the split makes a formatting, plan, and apply failure distinguishable in the checks list, and leaves a seam for a future GitHub Environment on `apply` (manual approval, deployment history) that a mid-job apply can never have.

**Rejected shape: `apply` as a `needs: plan` job applying the plan job's saved `tfplan`.**
The binary plan file embeds every input variable in plaintext — the provider token and every `tf_secret_vars` value included; `sensitive = true` redacts display output, not the plan file — so carrying it between jobs means publishing the credentials as a downloadable artifact, the secrets-at-rest [ADR-008](../decisions/008-secret-terraform-variables.md) exists to avoid.
...
Record the decision in an ADR at implementation time: it completes ADR-011's deliberate carve-out, and the rejected `needs` shape belongs in its alternatives.
...
- **Normalising job ids/display names across families** — **rejected here, then reversed by [ADR-011](../decisions/011-reusable-job-ids-are-the-check-name.md); it rode the v2 cut rather than this plan.**
  ...
  A 2026-09-19 audit of the post-v2 workflows found one residue of the inconsistency still standing — the degenerate `terraform / terraform` context ADR-011 deliberately spared — and it is now [scope item 6](#6-terraformyml-split-the-terraform-job-into-format-plan-and-apply), so nothing of the original inconsistency outlives v3.
- **A separate `validate` job in [scope item 6](#6-terraformyml-split-the-terraform-job-into-format-plan-and-apply)'s split** — considered alongside `format` when that check was pulled out into a job of its own ([#27](https://github.com/flungo/github-workflows/pull/27) review, 2026-09-20), and rejected: `plan` subsumes it, because a plan over a configuration `terraform validate` would reject fails on the same diagnostics.
  A third state-free job would spend a check context and a runner on a signal the split already carries, so it stops at `format`, `plan` and `apply`.
...
This plan renames no product filenames and touches the job structure of exactly **one workflow**: [scope item 6](#6-terraformyml-split-the-terraform-job-into-format-plan-and-apply) splits `terraform.yml`'s single job, so the required set changes from `terraform / terraform` to `terraform / plan` + `terraform / format`, and push runs report `terraform / apply`.
...
| --- | --- | --- |
| `terraform-github` | `terraform.yml`, `markdown-lint.yml`, `markdown-links.yml`, `markdown-sembr.yml`, `flungo-workflows.yml` callers | Move `provider_token`/`tf-var-name` into the map as `"github_token"`; rename `tf_secret_vars:` → `TF_SECRET_VARS:`; bump 5 pins. Also owns scope item 6's strings: update `standard-repository`'s required set to `terraform / plan` + `terraform / format` and apply it per repo immediately before each Terraform consumer's migration PR merges. The `surface-classic-protection` follow-on job is artifact-contract only — untouched. |
| `terraform-grafana-cloud` | `terraform.yml`, `terraform-drift.yml`, `flungo-workflows.yml` callers | Same provider-token move in both callers (`"grafana_cloud_access_policy_token"` entry); `force_run:` → `force-run:` in the drift caller (the caller's own `workflow_dispatch` input name is its own business); bump 3 pins; required set flips to `terraform / plan` + `terraform / format` immediately before merge. |
...
1. One PR, on an ordinarily-named feature branch (never a branch matching `v[0-9]*` — creation is restricted to the release App and the rejection message won't say why), carrying:
   ...
   - a draft **`## v3` section in [`upgrading.md`](../reference/upgrading.md)** ([ADR-013](../decisions/013-per-major-upgrade-guide.md)), carrying a settling note as the v2 section did before its cut, and naming the required-set change (`terraform / terraform` → `terraform / plan` + `terraform / format`, with `terraform / apply` new on push runs) and the required-check ordering it needs;
...
- [ ] `terraform-github`'s `standard-repository` required set updated to `terraform / plan` + `terraform / format`, rolled out per repo with the migrations below
