...
| [`markdown-validation.md`](markdown-validation.md) | The Markdown lint/link/semantic-line-break workflows — repo-agnostic; what each one is worth, and the markdownlint rule choices that pair with them |
| [`upgrading.md`](upgrading.md) | What a consumer must do to move between pinned majors — breaking changes only, one section per major |
