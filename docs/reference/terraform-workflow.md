...
- **Extra secret variables** the config itself needs — e.g. tokens it *distributes* to other repos — come through the **`tf_vars`** secret: a JSON object `{"<var>": "<value>", …}` (single-line string values) that the workflow explodes into masked `TF_VAR_<var>` env vars, naming no specific variable. A caller assembles the map inline from its own standalone secrets, so each value stays an independently-rotatable secret — e.g. `tf_vars: '{"lychee_github_token": ${{ toJSON(secrets.LYCHEE_GITHUB_TOKEN) }}}'`. **Declare the consuming Terraform variable `sensitive = true`**: `::add-mask::` keeps each value out of the run *logs*, but the plan text is also written to the `terraform-plan` artifact and posted into the PR comment (and, from drift, into the `drift` issue), none of which pass through log masking — only `sensitive = true` keeps a value out of plan output. (The provider marks `github_actions_secret.plaintext_value` sensitive itself; an arbitrary variable is not.) The same `tf_vars` secret exists on `terraform-drift.yml`, so a drift-enabled repo mirrors it there too.
