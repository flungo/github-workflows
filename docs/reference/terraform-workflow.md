...
- **Extra secret variables** the config itself needs — e.g. a database password or an API key a resource is configured with — come through the **`tf_vars`** secret: a JSON object `{"<var>": "<value>", …}` of *secret* values only (string values; multi-line is fine) that the workflow explodes into masked `TF_VAR_<var>` env vars, naming no specific variable. A caller assembles the map inline from its own standalone secrets, so each value stays an independently-rotatable secret — e.g. `tf_vars: '{"db_password": ${{ toJSON(secrets.DB_PASSWORD) }}}'`. **Declare the consuming Terraform variable `sensitive = true`**: `::add-mask::` keeps each value out of the run *logs*, but the plan text is also written to the `terraform-plan` artifact and posted into the PR comment (and, from drift, into the `drift` issue), none of which pass through log masking — only `sensitive = true` keeps a value out of plan output (an arbitrary input variable is not sensitive on its own). The same `tf_vars` secret exists on `terraform-drift.yml`, so a drift-enabled repo mirrors it there too.
