...
| [`adopting-markdown-workflows.md`](adopting-markdown-workflows.md) | How any repo adopts the Markdown lint/link workflows — callers, required permissions, per-repo config, token provisioning, verifying each check, and the adoption pitfalls; what adopting means in a repo managed by `terraform-github`, whose `markdown` flag makes the caller job names a cross-repo contract; plus the optional `markdown-standards` plugin for Fabrizio's own conventions |
