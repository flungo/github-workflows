...
| [`adopting-markdown-workflows.md`](adopting-markdown-workflows.md) | How any repo adopts the Markdown lint/link workflows — callers, per-repo config, the check-then-fix commit procedure, the semantic-line-break reflow, and adopting the `markdown-standards` plugin for the Claude-facing conventions |
