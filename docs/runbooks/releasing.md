...
Then, on each consumer, migrate `@v1` → `@v2` when you're ready and have accommodated the breaking change. Consumers move deliberately — nothing is pushed onto them. Opted-in consumers raise their own migration reminder, but not immediately — see [The grace period](#the-grace-period) below, then [Tracking consumer migration](#tracking-consumer-migration).

## The grace period

**For 7 days after a major is cut, its contract is not yet final and nobody is prompted onto it.** Rationale: [ADR-014](../decisions/014-grace-period-before-prompting-a-new-major.md).

Two things follow:

- **No consumer is told to migrate.** The [version check](#tracking-consumer-migration) prompts against the newest major whose grace period has passed, so an opted-in consumer on `@v1` hears nothing about `v2` until day eight.
- **The new major can take a further breaking change in place.** Land it on `main` as an ordinary merge — `release.yml` advances the new major onto it. **No `MAJOR_BRANCH` bump**, because nobody has been asked to move yet.

This is what makes cutting a major incremental. The contract is proved by adopting it, one repository at a time, and what that turns up — a check name that reads badly in a checks list, a required-check string that had to change in an order nobody predicted — arrives after the cut. Without the window, the only options are one enormous pull request that gets the whole major right before cutting it, or a `v3` because `v2` got a name slightly wrong.

Use it that way: **cut the major, then migrate your own repositories inside the window**, and fix what that finds on the new major directly.

What it does *not* change:

- **The old major froze at the cut**, not at the end of grace. `release.yml` stopped advancing it the moment `MAJOR_BRANCH` changed. Grace delays the prompt, not the freeze.
- **A settling change is still a breaking change.** If it changes what the [upgrade guide](../reference/upgrading.md) says a consumer must do, update that major's section in the same PR — the section stays editable for the week, and is a record afterwards.
- **Anyone who adopts inside the window accepts in-place changes.** In practice that is you, migrating the fleet.

After 7 days the contract is final and the normal rules resume: the next breaking change cuts the next major.
...
1. **Check the date of the last cut first.** If the current major was cut less than 7 days ago, you are inside [the grace period](#the-grace-period): fix it on `main`, update the upgrade guide's section for that major, and stop. No new major.
2. **Otherwise prefer fixing it forward.** If compatibility can be restored on `main` — re-add the removed input as optional, reinstate the old default — do that. The next merge advances the fix onto `@v<current>` and no new major is needed. Cutting a major forces *every* downstream repo to migrate, so avoid it unless the change genuinely can't be reconciled.
...
The comparison is against the newest major **out of [grace](#the-grace-period)**, so the first issues appear on the eighth day after a cut, not the first — and a repository lagging two majors is pointed at the settled one, not the one still moving. The check dates the cut itself, from the oldest commit the new major has that the previous one does not; there is nothing to record or maintain at cut time.
